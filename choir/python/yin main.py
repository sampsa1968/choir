...
def differenceFunction(x, N, tau_max):
    ...
    fcco = fc.conjugate()
    fcmu = fc * fcco;
    conv1 = np.fft.irfft(fcmu)
    conv = conv[:tau_max]
    vec = x_cumsum[w:w - tau_max:-1] + x_cumsum[w] - x_cumsum[:tau_max];
    return vec - 2 * conv
...
def compute_yin(sig, sr, dataFileName=None, w_len=512, w_step=256, f0_min=100, f0_max=500, harmo_thresh=0.1):
    ...
    :param harmo_tresh: Threshold of detection. The yalgorithm? return the first minimum of the CMND fubction below this treshold.
...
def main():
    ...
    audioFileName="/Users/SampsaLaine/Music/minne monne manne 16 signed.wav"
    w_len=1024
    w_step=256
    f0_min=70
    f0_max=200
